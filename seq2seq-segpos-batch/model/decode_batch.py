...
class Decode(nn.Module):
    ...
    def forward(self, encode_out, var_b, list_b, mask_v, length, is_train=False):
        chars_var = var_b[0]
        ...
        for idx in range(max_length):
            ...
            output = output.transpose(0, 1)
            self.action_batch(idx, states, output, length, train=is_train)
    ...
    def wordlstm_cell_batch(self, index, states, encode_out, masks, h, c, batch_size):
        if index == 0:
            ...
            #     last_pos_cur.append(states.pos_index[id][-1])
    ...
    def action_batch(self, index, states, output, length, train=False):
        if train:
    ...
    def wordlstm_cell(self, sent, encode_sent, index, h, c):
        if index == 0:
            ...
            if last_wordlen > self.wordlen_max: last_wordlen = self.wordlen_max+1
